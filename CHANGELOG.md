...
standard input, publishes the two libraries the tool is built on as packages of their
own, and fixes a long list of IDL translation and code generation defects.
...
- `codegen` types an `array` as `IList<T>` rather than `List<T>`. A `List<T>` still satisfies
  ...
  nor from.
- `codegen` opens every generated file with `#nullable enable` and declares `Get` as
  returning `object?`. The output is nullable-annotated throughout, and an annotation
